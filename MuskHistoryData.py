...
mainData = mainData.groupby('Tweet_Date').agg(
    {'Likes': 'sum', 'Retweets': 'sum', 'Reply_Count': 'sum', 'Sentiment_Score': 'mean'})

# mainData.reset_index().head()

# %%
# Get the data of the stock Tesla Stock (TSLA)
stockData = yf.download("TSLA", start="2019-4-1", end="2020-02-26")
...
scaled_df = pd.DataFrame(scaled_df, columns=names, index=index)
...
# Use date string to set xaxis range
fig.update_layout(xaxis_range=['2019-5-1', '2020-2-26'],title_text="Tesla's Stock Price vs Sentiment Score")
fig.show()
...
# Use date string to set xaxis range
fig.update_layout(xaxis_range=['2019-5-1', '2020-2-25'],title_text="Tesla's Stock Price vs Likes, Replies, & Retweets")
fig.show()


# %%
#Likes, Replies, and Retweets vs Sentiment
fig = go.Figure()
...
# Use date string to set xaxis range
fig.update_layout(xaxis_range=['2019-5-1', '2020-2-25'],title_text="Sentiment Score vs Likes, Replies, & Retweets")
fig.show()
